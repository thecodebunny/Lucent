...
if ( !class_exists( 'ReduxFramework_wbc_importer' ) ) {
    ...
    class ReduxFramework_wbc_importer {
        ...
        public function render() {
            ...
            echo '<div class="theme-display"><div class="themes">';
            ...
            if ( !empty( $this->field['wbc_demo_imports'] ) ) {
                ...
                foreach ( $this->field['wbc_demo_imports'] as $section => $imports ) {
                    ...
                    if ( !array_key_exists( 'imported', $imports ) ) {
                        ...
                        $imported = false;
                        $import_message = esc_html__( 'Import Demo', 'lucent' );
                    }else {
                        $imported = true;
                        $extra_class = 'active imported';
                        $import_message = esc_html__( 'Demo Imported', 'lucent' );
                    }
                    ...
                    if ( false == $imported ) {
                        echo '<div class="wbc-importer-buttons"><span class="spinner">'.esc_html__( 'Please Wait...', 'lucent' ).'</span><span class="button-primary importer-button import-demo-data">' . __( 'Import Demo', 'lucent' ) . '</span></div>';
                    }else {
                        echo '<div class="wbc-importer-buttons button button-secondary importer-button">'.esc_html__( 'Imported', 'lucent' ).'</div>';
                        echo '<span class="spinner">'.esc_html__( 'Please Wait...', 'lucent' ).'</span>';
                        echo '<div id="wbc-importer-reimport" class="wbc-importer-buttons button-primary import-demo-data importer-button">'.esc_html__( 'Re-Import', 'lucent' ).'</div>';
                    }
                    echo '</div>';
                    echo '</div>';
                ...
                echo "<h5>".esc_html__( 'No Demo Data Provided', 'lucent' )."</h5>";
            }
